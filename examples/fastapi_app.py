...
from llmbridge.schemas import Message
...
async def lifespan(app: FastAPI):
    ...
    if llmbridge.db:
        ...
        result = await llmbridge.db.apply_migrations()
        print(f"✅ Database connected and migrations applied: {result}")

        # Get initial pool stats
        pool_stats = await llmbridge.db.get_pool_stats()
...
async def get_llmbridge() -> LLMBridge:
...
async def health_check(service: LLMBridge = Depends(get_llmbridge)):
...
@app.get("/metrics")
async def get_metrics(service: LLMBridge = Depends(get_llmbridge)):
...
async def chat_completion(
    request: ChatRequest, service: LLMBridge = Depends(get_llmbridge)
):
...
async def list_models(
    provider: str = None, service: LLMBridge = Depends(get_llmbridge)
):
...
async def get_usage_stats(
    user_id: str, days: int = 30, service: LLMBridge = Depends(get_llmbridge)
):
