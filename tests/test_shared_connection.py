"""Test shared connection mode for LLMDatabase and LLMBridge."""
...
async def test_llm_database_with_external_manager():
    ...
    try:
        ...
        db = LLMDatabase.from_manager(shared_manager, schema="test_llmbridge")
...
@pytest.mark.asyncio
async def test_llmbridge_with_shared_connection():
    """Test LLMBridge with shared database connection."""
    # Create a shared manager
    config = DatabaseConfig(
        connection_string="postgresql://postgres:postgres@localhost/postgres",
        max_connections=15,
    ...
    try:
        # Create LLMBridge with shared manager
        service = LLMBridge(
