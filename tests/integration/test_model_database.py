...
@pytest.mark.asyncio
class TestLLMBridgeDatabaseIntegration:
    """Integration tests for LLMBridge database operations."""

    @pytest.fixture
    async def llmbridge(self, llm_test_db):
        """Create LLMBridge with database for testing."""
        service = LLMBridge(
            db_connection_string=llm_test_db["db_url"],
            origin="test-llmbridge",
    ...
    @pytest.fixture
    async def sample_models(self, llmbridge):
        """Create sample models for testing."""
        # Ensure database is initialized
        await llmbridge._ensure_db_initialized()
        ...
        for model in test_models:
            await llmbridge.db.add_model(model)

        return test_models

    async def test_get_models_from_db_all(self, llmbridge, sample_models):
    ...
    async def test_get_models_from_db_with_inactive(self, llmbridge, sample_models):
        """Test fetching models including inactive ones."""
        # Test fetching all models including inactive
        models = await llmbridge.get_models_from_db(active_only=False)
    ...
    async def test_get_models_from_db_by_provider(self, llmbridge, sample_models):
        """Test fetching models filtered by provider."""
        # Test fetching OpenAI models only
        openai_models = await llmbridge.get_models_from_db(provider="openai")
        ...
        anthropic_models = await llmbridge.get_models_from_db(provider="anthropic")
    ...
    async def test_get_model_from_db(self, llmbridge, sample_models):
        """Test fetching a specific model from database."""
        # Test fetching existing model
        model = await llmbridge.get_model_from_db("openai", "test-gpt-4")
    ...
    async def test_get_model_from_db_not_found(self, llmbridge, sample_models):
        """Test fetching a non-existent model."""
        # Test fetching non-existent model
        model = await llmbridge.get_model_from_db("openai", "non-existent-model")

        assert model is None

    async def test_get_model_from_db_inactive(self, llmbridge, sample_models):
        """Test that inactive models are not returned."""
        # The get_model method in db.py filters by is_active=TRUE
        model = await llmbridge.get_model_from_db("openai", "test-gpt-3.5-inactive")
    ...
    async def test_concurrent_model_fetches(self, llmbridge, sample_models):
        ...
        agents = [
            ...
            llmbridge.get_models_from_db(active_only=False),
        ]
